    ...
    def data_plotter(self, **kwargs):
        ...
        **kwargs (dictionary)
        ...
        if os.path.exists(self.white_noise_file) and self.white_noise_file[-4:] == '.wav':
            ...
            axins1.set_xlabel('Time', c=axins_color)
            axins1.set_xticks([])
            axins1.set_yticks([])
            axins1.axhline(y=-.6, xmin=.05, xmax=.05+(5e3/2e4), c=raw_data_color)
            axins1.text(x=1e3, y=-.8, s='10 ms')
            axins1.text(x=2e2, y=.8, s='Raw signal')

            # plot magnitude inset
            axins2 = inset_axes(ax1, width='33%', height='33%', loc=1)
            axins2.plot(xf_plot[:300000], yf_plot[:300000], c=raw_data_color)
            axins2.set_xlabel('Frequency (kHz)', c=axins_color)
            axins2.set_ylabel('Magnitude (a.u.)', c=axins_color)
            axins2.tick_params(axis='both', colors=axins_color)
            axins2_xticks = np.arange(0, 6000, 1000)
            axins2.set_xticks(axins2_xticks)
            axins2.set_xticklabels([f'{int(x_label / 1e3)}' for x_label in axins2_xticks])
            axins2.set_yticks([])
            if save_fig:
