...
def check_curve_exceeds_shuffled(curve1d, shuffled_mean, shuffled_std,  min_acc_rate=True, bin_radius_to_check=1):
    ...
    ----------
    truth_value : bool
        Whether the 1-D tuning curve exceeds shuffled.
    ----------
    """
    if (min_acc_rate is True or curve1d.max() > min_acc_rate) and curve1d.max() > shuffled_mean[np.argmax(curve1d)] + 3*shuffled_std[np.argmax(curve1d)]:
